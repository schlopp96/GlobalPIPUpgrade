...
def get_outdated_pkgs():
    ...
    try:
        ...
        get_outdated = subprocess.run(cmd,
                                      check=True,
...
def upgrade_outdated(outdated_pkgs: list):
    ...
    for count, i in enumerate(outdated_pkgs, start=1):
        ...
        try:
            ...
            upgrade_outdated = subprocess.run(cmd,
                                              check=True,
                                              stdout=subprocess.PIPE,
                                              stderr=subprocess.STDOUT)
        except subprocess.CalledProcessError as err:
...
def upgrade_all():
    ...
    script_p = subprocess.Popen(
        ['powershell.exe', './scripts/force_upgrade_pip_pkgs.ps1'],
        stdout=subprocess.PIPE,
        stderr=subprocess.STDOUT)
    with script_p.stdout:
        try:
            ...
            mainLogger.warning(
                f'An error occurred during execution of "upgrade_all" subprocess:\n--> {str(err)}'
            )
            print(
                f'An error occurred during execution of "upgrade_all" subprocess:\n--> {str(err)}'
            )
